...
class LLamaServerMonitor:
    ...
    async def start_server(self):
        ...
        if self.state == ServerState.RUNNING:
            logger.info(f"[INFO] Server already running (PID: {self.pid})")
            return
        
        logger.info(f"[INFO] Starting llama-server...")
        ...
        try:
            ...
            self.running = True
            logger.info(f"[INFO] Server started with PID: {self.pid}")
    ...
    async def wait_for_ready(self, timeout: int = 15):
            ...
            try:
                ...
                async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=3)) as session:
                    async with session.get(f"http://{self.config['host']}:{self.config['port']}/v1") as resp:
                        if resp.status == 200:
                            logger.info("[INFO] Server is ready and healthy")
                            return True
            except Exception as e:
                pass
        ...
        logger.info(f"[WARN] Server ready timeout ({timeout}s)")
        return False
    ...
    async def check_and_restart(self):
        ...
        if not self.is_process_alive():
            logger.info(f"[WARN] Server process died (PID: {self.pid}), restarting...")
        ...
        if not is_healthy:
            logger.info("[WARN] Server health check failed, restarting...")
            await self.stop_server()
            await asyncio.sleep(self.config["restart_delay"])
            await self.start_server()
    ...
    async def process_request(self, prompt: str, max_tokens: int = 1024, model: str = "uncensored"):
        ...
        async with self.lock:
            if not self.running or self.state != ServerState.RUNNING:
                logger.info("[WARN] Cannot process request: server not running")
                return
            ...
            while retry_count < max_retries:
                    ...
                    async with aiohttp.ClientSession(timeout=timeout) as session:
                        async with session.post(
                            ...
                            if resp.status == 200:
                                ...
                                logger.info(f"[SUCCESS] Request completed in {log.duration:.2f}s "
                ...
                if retry_count < max_retries:
                    ...
                    logger.info(f"[INFO] Retry {retry_count}/{max_retries} in {wait_time}s")
    ...
    def add_request(self, prompt: str, max_tokens: int = 1024, model: str = "uncensored"):
        ...
        logger.info(f"[INFO] Request queued. Queue size: {len(self.request_queue)}")
    
    async def run(self):
        """主循环"""
        logger.info("=" * 60)
        logger.info("LLaMA Server Monitor Started")
        logger.info(f"Platform: {platform.system()} {platform.release()}")
        logger.info(f"Service mode: {'Enabled' if self.is_service_available() else 'Not available'}")
        logger.info("=" * 60)
        ...
        try:
            ...
            logger.exception(f"[ERROR] Critical error: {e}")
        finally:
    ...
    def _shutdown(self):
        ...
        if self.process:
            self.stop_server()
        self.state = ServerState.STOPPED
        logger.info("[INFO] Monitor shutdown complete")
...
def main():
    ...
    if args.start:
        logger.info("[INFO] Starting llama-server...")
        asyncio.run(monitor.start_server())
        logger.info("[INFO] Server started successfully")
    
    elif args.stop:
        logger.info("[INFO] Stopping llama-server...")
        asyncio.run(monitor.stop_server())
        logger.info("[INFO] Server stopped")
    
    elif args.status:
        logger.info(f"State: {monitor.state.value}")
        logger.info(f"PID: {monitor.pid}")
        logger.info(f"Running: {monitor.running}")
        logger.info(f"Queue size: {len(monitor.request_queue)}")
