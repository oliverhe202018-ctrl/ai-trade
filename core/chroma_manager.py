...
def get_chroma_client():
    ...
    with _thread_lock:
        ...
        try:
            ...
            logger.exception(f"[ERROR] ChromaDB 客户端初始化失败: {e}")
            raise
...
def get_trading_memory_collection():
    ...
    try:
        ...
        logger.exception(f"[ERROR] 获取 trading_memory 集合失败: {e}")
        raise
...
def add_trading_experience(experience_data: dict, doc_id: str):
    ...
    try:
        ...
        logger.exception(f"[ERROR] 添加交易经验失败: {e}")
        raise
...
def query_trading_experience(query_text: str, top_k: int = 2):
    ...
    try:
        ...
        logger.exception(f"[ERROR] 查询交易经验失败: {e}")
