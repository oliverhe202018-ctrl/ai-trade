...
from core.risk_manager import check_building_cooldown, calculate_atr, calculate_position_size
...
def _load_cache(code, days):
    ...
    try:
        ...
        logger.info(f"  [CACHE HIT] {code} 命中本地缓存，跳过网络请求")
        return records
    except Exception as e:
        logger.exception(f"  [CACHE WARN] {code} 缓存读取失败: {e}")
        return None
...
def _save_cache(code, days, records):
    ...
    try:
        ...
        logger.exception(f"  [CACHE WARN] {code} 缓存落盘失败: {e}")
...
def download_historical_data(code, days=100):
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                logger.warning(f"[重试] 网络波动，2秒后重试... (第{attempt + 1}次失败: {e})")
                time.sleep(2)
            else:
                logger.warning(f"[WARN] 腾讯财经接口彻底异常 {code}: {e}，跳过该标的")
                return code, [], True
    ...
    try:
        ...
        if not klines:
            logger.warning(f"[WARN] 腾讯接口返回空数据 {code}")
            return code, [], True
        ...
        logger.exception(f"[WARN] 腾讯财经接口数据解析异常 {code}: {e}，跳过该标的")
        return code, [], True
...
def run_backtest(days=30, initial_cash=100000):
    ...
    for code in BACKTEST_UNIVERSE:
        ...
        try:
            ...
            logger.exception(f"[WARN] 获取 {code} 失败: {e}，跳过该标的")
            continue

    dates = sorted(list(market_history.keys()))
    if not dates:
        logger.info("数据下载全部失败，请检查网络或稍后再试。")
        return

    logger.info(f"数据清洗完毕，共计 {len(dates)} 个交易日 (其中 {network_request_count} 次真实网络请求)。启动回放...\n")
    ...
    logger.info(f"[ROUTER] 动态策略路由已启用 | 网格步长={grid_cfg.get('step_pct', 0.03)*100}% | 每格金额={grid_cfg.get('trade_amount', 5000)}")
    ...
    logger.info(f"[预热] 已用前 {warmup_days} 天数据填充 stock_history，ATR 指标可正常计算")
    ...
    for date_idx, date in enumerate(dates[warmup_days:]):
        ...
        if pending_orders:
            logger.info(f"执行昨日待执行订单 {len(pending_orders)} 笔 (使用今日开盘价)...")
            executed_orders = []
            ...
            for order in pending_orders:
                ...
                if action == "SELL":
                    ...
                    if portfolio["cash"] >= total_cost:
                        ...
                        logger.info(f"  [跳过] {code} 资金不足: 需要 ¥{total_cost:.2f}, 可用 ¥{portfolio['cash']:.2f}")
        ...
        for sell in sells:
            ...
            logger.info(f"  [信号] 生成卖出订单: {sell['code']} (信号价: {sell['price']:.2f})")
        ...
        if regime_summary:
            logger.info(f"  [ROUTER] regime 分布: {regime_summary}")
            ...
            for q in regime_buckets["grid"]:
                ...
                for signal in grid_signals:
                    ...
                    if signal["action"] == "BUY":
                        ...
                        logger.info(f"  [GRID] {code} 触发网格买入 (L{signal['grid_level']}) @ {signal['price']:.2f}")
                        ...
                        if held_shares >= shares:
                            ...
                            logger.info(f"  [GRID] {code} 触发网格卖出 (L{signal['grid_level']}) @ {signal['price']:.2f}")
                        else:
                            logger.info(f"  [GRID] {code} 持仓不足，跳过网格卖出")
        ...
        if regime_buckets["smart_dca"]:
            ...
            if dca_cfg:
                ...
                for q in regime_buckets["smart_dca"]:
                    ...
                    if multiplier <= 0:
                        logger.info(f"    [DCA] {code} 乘数=0，暂停定投")
                        continue
                    ...
                    logger.info(f"    [DCA] {code} 定投 {shares}股 (乘数{multiplier}, 金额{dca_amount:.0f}, ATR={atr:.2f})")
        ...
        if regime_buckets["trend"]:
            ...
            for buy in decisions.get("buys", []):
                ...
                logger.info(f"  [信号] 生成趋势买入订单: {code} {shares}股 (信号价: {price:.2f}, ATR={atr:.2f})")
            ...
            for mr in mr_candidates:
                ...
                logger.info(f"  [信号] 生成均值回归买入订单: {mr.stock_code} {shares}股 (信号价: {current_price:.2f}, ATR={atr:.2f})")
        ...
        if cooldown_limit < max_positions:
            ...
            if len(buy_orders) > cooldown_limit:
                ...
                logger.info(f"[建仓冷却] 限制当日开仓数至 {cooldown_limit}（原 {original_buy_count} 笔买入信号）")
        ...
        logger.info(f"当日收盘净值: {total_equity:.2f} (仓位: {daily_market_value/total_equity*100:.1f}%)")
        logger.info(f"待执行订单队列: {len(pending_orders)} 笔")
    ...
    if pending_orders:
        ...
        for order in pending_orders:
            logger.info(f"  - {order['action']} {order['code']} (信号价: {order['signal_price']:.2f})")

    # 4. 生成绩效报告
    logger.info("\n" + "=" * 50)
    logger.info("大模型量化回测报告 (LLM Backtest Report)")
    logger.info("=" * 50)
    ...
    logger.info(f"测试区间: {dates[0]} 至 {dates[-1]}")
    logger.info(f"初始资金: {initial_cash:.2f}")
    logger.info(f"最终净值: {final_equity:.2f}")
    logger.info(f"区间收益率: {total_return:+.2f}%")
    logger.info(f"最大回撤: -{max_drawdown:.2f}%")
    logger.info(f"总交易次数: {len(trade_log)}")
    logger.info(f"交易成本: 印花税{STAMP_DUTY*100:.2f}% 佣金{COMMISSION*100:.3f}% 滑点{SLIPPAGE*100:.2f}%")
    logger.info("=" * 50)
