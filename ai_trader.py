...
from broker import get_broker, load_config
from market_data import get_realtime_quotes
from strategy_engine import select_stocks, generate_sell_signals
...
def ai_decide(mode="mock", strategy_override=None):
    ...
    print("=" * 70)
    print("🤖 AI 交易 Agent v3 - 启动")
    print(f"   交易模式: {mode}")
    print(f"   启动资金: ¥{config.get('initial_capital', 50000):,.0f}")
    print(f"   日目标利润: ¥{config.get('daily_target', 50)}")
    print(f"   策略: {strategy_override or config.get('strategy', {}).get('type', 'trend')}")
    print("=" * 70)

    # 连接券商
    print("\n[步骤0] 连接券商...")
    try:
        # 优先用命令行参数，否则从 config.yaml 读取
        config = load_config()
        effective_mode = args.mode if args.mode != "mock" else config.get("broker_mode", "mock")
        broker = get_broker(effective_mode)
        connect_result = broker.connect()
        if connect_result.get("status") == "error":
            print(f"  ❌ 券商连接失败: {connect_result.get('msg')}")
            print(f"  → 回退到模拟盘")
            broker = get_broker("mock")
            broker.connect()
        else:
            print(f"  ✅ 已连接: {connect_result.get('broker')}")
            if connect_result.get("path"):
                print(f"  📁 路径: {connect_result['path']}")
    except Exception as e:
        print(f"  ⚠️ 券商初始化失败: {e}，回退到模拟盘")
        broker = get_broker("mock")
        broker.connect()
    ...
    if positions:
        for code in positions:
            ...
            if pos_info:
                print(f"    {code}: {pos_info.get('shares', '?')}股")
            else:
                print(f"    {code}: ?")
    ...
    if quotes:
        ...
        for i, q in enumerate(sorted_quotes[:10], 1):
            ...
            if q.get('limit_up'):
                ...
                flag = "🟢跌停"
            print(f"  {i:2d}. {q['name']:10s} ¥{q['price']:>8.2f}  {sign}{q.get('change_pct', 0):.2f}%  [{q.get('sector', '')}] {flag}")
        print(f"\n  ... (共 {len(quotes)} 只)")
    ...
    if trades:
        ...
        for t in trades:
            print(f"   📥 {t['name']}({t['code']}) ¥{t['price']:.2f} x {t['shares']}股")
            print(f"      理由: {t['reason']}")
    else:
        print("\n🤷 AI 认为当前市场没有合适的买入机会")
    ...
    for t in sell_trades:
        ...
        print(f"  卖出: {json.dumps(result, ensure_ascii=False, indent=2)}")
    
    for t in trades:
        result = broker.buy(t['code'], t['price'], t['shares'])
        print(f"  买入: {json.dumps(result, ensure_ascii=False, indent=2)}")

    # 最终报告
    print(f"\n[步骤5] 最终账户状态:")
    final_accounts = broker.get_accounts()
    if final_accounts:
        print(json.dumps(final_accounts, ensure_ascii=False, indent=2))
    ...
    if trades:
        for t in trades:
            print(f"  ✅ 买入 {t['name']}({t['code']}) @ ¥{t['price']:.2f} x {t['shares']}股")
    if sell_trades:
        for t in sell_trades:
            print(f"  💸 卖出 {t['name']}({t['code']}) @ ¥{t['price']:.2f} x {t['shares']}股")
