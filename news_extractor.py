    ...
    try:
        ...
        for item in feed_list:
            ...
            try:
                ...
                print(f"[WARN] 解析资讯时间失败: {e}")
                continue

        print(f"[NEWS] 获取到 {len(filtered_news)} 条近 {hours} 小时资讯")
        return filtered_news

    except Exception as e:
        print(f"[ERROR] 获取新浪 7x24 资讯失败: {e}")
        return []
...
def extract_sentiment_with_llm(news_list):
    ...
    try:
        ...
        print(f"[WARN] 情绪因子 JSON 解析失败: {e}")
    except Exception as e:
        print(f"[WARN] 情绪因子提取异常: {e}")
