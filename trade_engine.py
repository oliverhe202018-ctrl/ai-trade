...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(
            os.path.join(LOG_DIR, f"trades_{datetime.now().strftime('%Y%m%d')}.log"),
            encoding="utf-8",
        ),
...
class MockBroker(BaseBroker):
    ...
    def __init__(self, state_file: str = None, lock_file: str = None, position_key: str = "position"):
    ...
    def buy(self, code: str, shares: int, price: float, **kwargs) -> dict:
        ...
        log_trade("BUY", code, name, price, shares, total_cost, strategy=strategy,
                  expected_price=expected_price, actual_fill_price=price)
    ...
    def sell(self, code: str, shares: int, price: float, **kwargs) -> dict:
        ...
        profit = (price - old_price) * shares - commission - stamp_tax - transfer_fee
        profit_pct = ((price - old_price) / old_price * 100) if old_price > 0 else 0
        ...
        log_trade("SELL", code, name, price, shares, total_revenue, profit=profit,
                  strategy=strategy)
...
def main():
    if len(sys.argv) < 2:
        print("用法:")
        print("  python trade_engine.py status              # 查看账户状态")
        print("  python trade_engine.py buy CODE NAME PRICE SHARES REASON")
        print("  python trade_engine.py sell CODE NAME PRICE SHARES REASON")
        print("  python trade_engine.py settle              # 收盘结算")
        return
    ...
    if cmd == "status":
        ...
        if len(sys.argv) < 7:
            print("用法: buy CODE NAME PRICE SHARES REASON")
            return
        result = buy_stock(
        ...
        if len(sys.argv) < 7:
            print("用法: sell CODE NAME PRICE SHARES REASON")
            return
        result = sell_stock(
            sys.argv[2], sys.argv[3],
            float(sys.argv[4]), int(sys.argv[5]),
